...
# Convertir 'time' en année en extrayant la partie entière (si c'est un flottant)
temperature['time'] = temperature['time'].apply(lambda x: int(np.floor(x)) if not np.isnan(x) else np.nan)
...
merged_data = pd.merge(temperature, puffins[['time', 'production_rate']], on="time")
